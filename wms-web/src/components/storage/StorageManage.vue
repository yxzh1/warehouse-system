...
export default {
  ...
  data(){
    let checkDuplicate = (rule,value,callback)=>{
      ...
      this.$axios.get(this.$httpsUrl+"/storage/findByName?name="+this.form.name).then(res=>res.data).then(res=>{
        if(res.code!=200){
          callback()
        }else {
  ...
  methods:{
    ...
    del(id){
      this.$axios.get(this.$httpsUrl+'/storage/del?id='+id).then(res=>res.data).then(res=>{
        if(res.code==200){
          this.$message({
            message: '操作成功',
            type: 'success'
    ...
    save(){
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$axios.post(this.$httpsUrl+'/storage/saveOrMod',this.form).then(res=>res.data).then(res=>{
            if(res.code==200){
              this.$message({
                message: '操作成功',
                type: 'success'
    ...
    loadPost(){
      this.$axios.post(this.$httpsUrl+'/storage/listPageP',{
